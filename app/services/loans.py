...
def calculate_late_fee(due_at: datetime, returned_at: datetime, price_cents: int) -> int:
    ...
    if returned_at <= due_at:
        return 0
    days_late = ceil((returned_at - due_at).total_seconds() / timedelta(days=1).total_seconds())
...
def list_member_loans(
    ...
    loan_outputs = [to_loan_out(loan, now) for loan in loans]
    if status is not None:
        loan_outputs = [loan for loan in loan_outputs if loan.status == status]
    return loan_outputs
